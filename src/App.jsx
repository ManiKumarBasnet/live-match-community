...
.stats-hero{padding:22px 24px;margin-bottom:18px;background:linear-gradient(135deg,rgba(7,16,29,.98),rgba(12,72,52,.94));color:#eef7ff;border:0;box-shadow:0 26px 56px rgba(7,16,29,.18)}.stats-hero-top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.stats-eyebrow{display:inline-flex;align-items:center;gap:8px;border-radius:999px;padding:6px 10px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);font-size:10.5px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;color:#b7c9d9}.stats-eyebrow svg{width:14px;height:14px;color:#5ee2a4}.stats-hero h2{font-family:var(--font-display);font-size:30px;line-height:1;letter-spacing:-.05em;margin:12px 0 10px}.stats-copy{max-width:640px;font-size:14px;line-height:1.6;color:#b6c7d7}.stats-badges{display:flex;flex-wrap:wrap;gap:8px;justify-content:flex-end;max-width:430px}.stats-badge{display:inline-flex;align-items:center;gap:8px;padding:9px 12px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1);font-size:12px;font-weight:850;color:#ecf3f9}.stats-badge svg{width:14px;height:14px;color:#5ee2a4}.stats-badge:nth-child(2) svg{color:#8bb8ff}.stats-badge:nth-child(3) svg{color:#f6c55e}
...
function Stats({ players, matches, stats }) {
  ...
  const renderOwnerLabel = (item) => {
    ...
    return [item.team, owner?.name].filter(Boolean).join(" • ");
  };
        ...
        <div className="metric-note">
          {item ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <span style={{ color: "var(--muted)", fontWeight: 750 }}>{renderOwnerLabel(item) || item.name}</span>
            </div>
              ...
              {current.length ? current.map((item, index) => {
                ...
                return (
                  <tr key={`${view}-${item.name}`}>
                    <td><div className="player-cell"><span className={`rank ${index === 0 ? "gold" : index === 1 ? "silver" : index === 2 ? "bronze" : ""}`}>{index + 1}</span>{index === 0 && <Crown size={17} color="var(--gold)" />}{(index === 1 || index === 2) && <Medal size={16} color={index === 1 ? "var(--silver)" : "var(--bronze)"} />}</div></td>
                    <td><div className="player-cell"><Avatar name={item.name} size={42} img={player?.avatar} /><div><div className="player-name">{item.name}</div><div className="player-meta"><Shield />{renderOwnerLabel(item) || item.name}</div></div></div></td>
